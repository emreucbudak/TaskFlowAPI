...
namespace ProjectManagement.Persistence.Data.Repositories
{
        ...
        ) : IProjectManagementWriteRepository
    {
        ...
        public async System.Threading.Tasks.Task AddIndividualTask(Domain.Entities.IndividualTasks entity)
        {
            if (entity == null)
            {
                logger.LogError("Eklenmeye çalışılan IndividualTask null");
                throw new ArgumentNullException(nameof(entity), "Entity null olamaz");
            }

            try
            {
                // Simple validation for now, expand as needed
                if (string.IsNullOrWhiteSpace(entity.TaskTitle))
                {
                    throw new ArgumentException("Task başlığı boş olamaz", nameof(entity));
                }
            ...
            {
                logger.LogError(ex, "Veritabanına IndividualTask eklenirken hata oluştu. Entity Id: {EntityId}", entity.Id);
                throw new InvalidOperationException("Veritabanına kayıt eklenirken bir hata oluştu", ex);
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "IndividualTask eklenirken beklenmeyen hata. Entity Id: {EntityId}", entity.Id);
                throw;
            }
        }

        public async System.Threading.Tasks.Task DeleteIndividualTask(Domain.Entities.IndividualTasks task)
        {
             if (task == null)
            {
                logger.LogError("Silinmeye çalışılan IndividualTask null");
                throw new ArgumentNullException(nameof(task), "Task null olamaz");
            }

            try
            {
                var existingEntity = await context.IndividualTasks
                    .FirstOrDefaultAsync(e => e.Id == task.Id);

                if (existingEntity == null)
                {
                    logger.LogWarning("Silinmek istenen IndividualTask bulunamadı. Id: {TaskId}", task.Id);
                    throw new InvalidOperationException($"Id'si {task.Id} olan kayıt bulunamadı");
                }
            ...
            {
                logger.LogError(ex, "IndividualTask silinirken eşzamanlılık hatası. Id: {TaskId}", task.Id);
                throw new InvalidOperationException("Kayıt başka bir işlem tarafından değiştirilmiş", ex);
            }
            catch (DbUpdateException ex)
            {
                logger.LogError(ex, "IndividualTask silinirken veritabanı hatası. Id: {TaskId}", task.Id);
                throw new InvalidOperationException("Kayıt silinirken veritabanı hatası oluştu", ex);
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "IndividualTask silinirken beklenmeyen hata. Id: {TaskId}", task.Id);
                throw;
            }
        }

        public async System.Threading.Tasks.Task UpdateIndividualTask(Domain.Entities.IndividualTasks task)
        {
            if (task == null)
            {
                logger.LogError("Güncellenmeye çalışılan IndividualTask null");
                throw new ArgumentNullException(nameof(task), "Task null olamaz");
            }
            ...
            try
            {
                ...
                {
                    throw new ArgumentException("Task başlığı boş olamaz", nameof(task));
                }
                ...
                if (existingEntity == null)
                {
                    logger.LogWarning("Güncellenmek istenen IndividualTask bulunamadı. Id: {TaskId}", task.Id);
                    throw new InvalidOperationException($"Id'si {task.Id} olan kayıt bulunamadı");
                }
            ...
            {
                logger.LogError(ex, "IndividualTask güncellenirken eşzamanlılık hatası. Id: {TaskId}", task.Id);
                throw new InvalidOperationException("Kayıt başka bir işlem tarafından değiştirilmiş", ex);
            }
            catch (DbUpdateException ex)
            {
                logger.LogError(ex, "IndividualTask güncellenirken veritabanı hatası. Id: {TaskId}", task.Id);
                throw new InvalidOperationException("Kayıt güncellenirken veritabanı hatası oluştu", ex);
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "IndividualTask güncellenirken beklenmeyen hata. Id: {TaskId}", task.Id);
                throw;
            }
        }
    }
