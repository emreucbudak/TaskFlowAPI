...
public sealed class RejectGroupActivityCommandHandler(
    IReadRepository<GroupActivity, Guid> readRepository,
    IIdentityCapUnitOfWork unitOfWork,
    ICurrentUserService currentUserService) : IRequestHandler<RejectGroupActivityCommandRequest>
{
    private const int GroupLeaderRoleId = 1;

    public async Task Handle(RejectGroupActivityCommandRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!isLeader)
            throw new AuthExceptions("Sadece grup liderleri aktiviteleri reddedebilir.");

        activity.Reject(currentUserId, request.Note);
